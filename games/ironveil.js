...
function loadLevel() {
  ...
  if (level < levels.length) {
    ...
    if (gen) {
      for (var gr = 0; gr < gen.length; gr++) {
        for (var gc = 0; gc < gen[gr].length; gc++) {
          var ch = gen[gr][gc];
          if (ch !== ".") addSprite(gc, gr, ch);
        }
      }
    }
  }
...
function canStep(x, y, avoidTypes) {
  var t = getTile(x, y);
  for (var i = 0; i < t.length; i++) {
    ...
    if (avoidTypes) {
      for (var j = 0; j < avoidTypes.length; j++) {
        if (tp === avoidTypes[j]) return false;
      }
    }
  }
  return true;
...
function stepTo(en, pl, avoidTypes) {
  ...
  var dy = pl.y - en.y;
  if (dx === 0 && dy === 0) {
    var esc = [[1,0],[-1,0],[0,1],[0,-1]];
    for (var ei = 0; ei < 4; ei++) {
      var ex2 = en.x + esc[ei][0];
      var ey2 = en.y + esc[ei][1];
      if (canStep(ex2, ey2, avoidTypes)) { en.x = ex2; en.y = ey2; return; }
    }
    return;
  }
  ...
  if (Math.abs(dx) >= Math.abs(dy)) {
    nx = en.x + ox;
    if (nx !== pl.x && canStep(nx, en.y, avoidTypes)) { en.x = nx; return; }
    if (oy !== 0) { ny = en.y + oy; if (ny !== pl.y && canStep(en.x, ny, avoidTypes)) { en.y = ny; return; } }
    nx = en.x - ox;
    if (canStep(nx, en.y, avoidTypes)) { en.x = nx; return; }
    if (oy !== 0) { ny = en.y - oy; if (canStep(en.x, ny, avoidTypes)) { en.y = ny; return; } }
  } else {
    ny = en.y + oy;
    if (ny !== pl.y && canStep(en.x, ny, avoidTypes)) { en.y = ny; return; }
    if (ox !== 0) { nx = en.x + ox; if (nx !== pl.x && canStep(nx, en.y, avoidTypes)) { en.x = nx; return; } }
    ny = en.y - oy;
    if (canStep(en.x, ny, avoidTypes)) { en.y = ny; return; }
    if (ox !== 0) { nx = en.x - ox; if (canStep(nx, en.y, avoidTypes)) { en.x = nx; return; } }
  }
}
...
function doSwordAttack(pl) {
  ...
  swordCD = 1;
  var hit = false;
  if (swordHitTile(pl.x, pl.y)) hit = true;
